import { TEXT_PLACEHOLDER, AMBER, TEXT_DARKEST, PANEL_LIGHT_BG, PANEL_STROKE_MID, PANEL_STROKE_FAINT, ARROW_COLOR } from '../../../styles/colors'
  ...
  return (
    <div style={{ padding: '1.5rem', overflowY: 'auto' }}>
      ...
      {rowConstructions.map((rc, i) => {
        ...
        const svgW = W + 70
        const widthArrowColor = hlEnds ? AMBER : ARROW_COLOR
          ...
          <div key={i} style={{ marginBottom: '2rem' }}>
            ...
            <svg width={svgW} height={totalH} style={{ display: 'block', overflow: 'visible' }}>
              ...
              <rect x={30} y={20} width={W} height={depthSc}
                fill={PANEL_LIGHT_BG} stroke={PANEL_STROKE_MID} strokeWidth="1.5" />
              {/* Hatch lines */}
              {Array.from({ length: Math.floor(W / 18) }, (_, k) => (
                <line key={k} x1={30 + k * 18} y1={20} x2={30 + k * 18 + depthSc * 0.6} y2={20 + depthSc}
                  stroke={PANEL_STROKE_FAINT} strokeWidth="0.5" />
              ))}
              {/* Trapezoid positions (vertical lines) */}
              {Array.from({ length: rc.numTrapezoids }, (_, j) => {
                const tx = 30 + (rc.railOverhang + j * rc.spacing) * sc
                return <line key={j} x1={tx} y1={20} x2={tx} y2={20 + depthSc} stroke={PANEL_STROKE_MID} strokeWidth="1.5" />
              })}
              {/* Label */}
              <text x={30 + W / 2} y={20 + depthSc / 2 + 4} fontSize="13" fontWeight="800" fill={TEXT_DARKEST}
              ...
              <line x1={30 + W + 10} y1={20} x2={30 + W + 10} y2={20 + depthSc} stroke={TEXT_DARKEST} strokeWidth="1" markerEnd="url(#arr)" markerStart="url(#arr)" />
              <text x={30 + W + 22} y={20 + depthSc / 2} fontSize="9" fontWeight="700" fill={TEXT_DARKEST} dominantBaseline="middle">{Math.round(panelDepth * 10)}</text>
            </svg>
          </div>
        )
      })}
