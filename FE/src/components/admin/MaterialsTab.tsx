...
import SadotEquipmentTab from './SadotEquipmentTab'
import { useLang } from '../../i18n/LangContext'

// Wrapper grouping Panels + Materials + Sadot equipment under one admin tab
// (mirrors the Users and Credits sub-tab structure).
export default function MaterialsTab() {
  const { t } = useLang()
  const [subTab, setSubTab] = useState<'panels' | 'materials' | 'sadot'>('materials')
  return (
    <div style={{ padding: '1rem 0' }}>
      <div style={{ display: 'flex', gap: '0.4rem', marginBottom: '1.1rem', borderBottom: `1px solid ${BORDER_FAINT}` }}>
        {[
          ...
          { key: 'sadot', label: t('admin.materials.subtab.sadot') },
        ].map(tab => (
          <button
            key={tab.key}
      ...
      {subTab === 'sadot' && <SadotEquipmentTab />}
    </div>
  )
}
