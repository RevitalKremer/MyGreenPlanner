...
export default function DetailView({ rc, trapId = null, panelLines = null, settings = {}, lineRails = null, highlightParam = null, beDetailData = null, fullTrapGhost = null, paramGroup: PARAM_GROUP = {}, reverseBlockPunches = true, onReset = null, onUpdateSetting = null, printMode = false }) {
  ...
  let legX1 = allLegXs[allLegXs.length - 1] ?? (x1 + OHx)
        ...
        <div style={{ transform: `translate(${panOffset.x}px, ${panOffset.y}px)`, transformOrigin: 'top left' }}>
          <div ref={contentRef} style={{
            ...
            <div style={{ fontSize: '0.78rem', fontWeight: '700', color: TEXT_SECONDARY, marginBottom: '0.75rem' }}>
              {trapId ?? `${rc.typeLetter}${rc.panelsPerSpan}`} — {angle}° · Panel Front {fmt(geom.panelFrontHeight ?? 0)} cm
              ...
              {innerLegXs.map((sx, ci) => {
                ...
                return (
                  <g key={ci}>
                    ...
                    {showAnnotations && <Dim ax1={sx} ay1={slopeTopY} ax2={sx} ay2={blockTopY} label={fmt(innerLegH)} off={14} />}
                  </g>
                )
              })}
