    ...
    <svg width={W} height={svgH} style={{ display: 'block', overflow: 'visible' }}>
      ...
      {showLabel && (
        <text x={(x0 + x1) / 2 - 6} y={(topY0 + baseY) / 2 - 4}
          fontSize="7" fill={TEXT_SECONDARY} fontStyle="italic" fontWeight="600"
      ...
      {/* Rear leg height */}
      {showLabel && hR > 0 && (
        <text x={x0 - 4} y={(topY0 + baseY) / 2} fontSize="7" fill={TEXT} textAnchor="end" dominantBaseline="middle" fontStyle="italic">{(heightRear / 100).toFixed(1)}</text>
      )}
      {/* Front leg height */}
      {showLabel && (
        <text x={x1 + 4} y={(topY1 + baseY) / 2} fontSize="7" fill={TEXT} textAnchor="start" dominantBaseline="middle" fontStyle="italic">{(heightFront / 100).toFixed(1)}</text>
      )}
      {/* Type label */}
